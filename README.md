...
1. Tạo thư mục gốc chứa dự án `fullstack`
2. Tạo thư mục `backend` và `frontend` trong thư mục `fullstack` như cấu trúc ở trên
3. Mở ứng dụng VS Code và mở thư mục `fullstack`

### Bước 2: Tạo ứng dụng server cho backend với Dart framework
1. Đi đến thư mục backend từ thư mục `fullstack`
    ```bash
    cd backend
    ```
